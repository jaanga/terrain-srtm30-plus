...
console.log( 'png', i, file );
	...
	function processFolderPNG( file) {
		var img = fs.readFile( __dirname + folderPNG + file, function( err, imageData ) {
			if ( err ) throw err;
...
*/
					...
					processTMSnew( fileTMS7, imageData, tmsWidth, pngWidth, xDelta, xOffset, yOffset, xStart, xFinish, yStart, yFinish, yDelta, tileX, tileY );
					yStart = yFinish;
				}
				xStart = xFinish;
				xFinish += xDelta;
...
console.log( 'processFolderPNG' , file, imageData );
		});
...
console.log('error', count, file );
			...
			.on('parsed', function () {
				...
				this.pack().pipe( fs.createWriteStream( __dirname + '/../../tms7/' + tileX + '/' + tileY + '.png' )  );
//				this.pack().pipe( fs.createWriteStream( __dirname + '/testa/test.png' )  );
console.log( 'tms', count, file, pngWidth, 'tw', tmsWidth, 'xo', xOffset, 'yo', yOffset, 'xs', xStart, 'xf', xFinish, 'ys', yStart, yFinish, 't', new Date() - startTimeApp);
				count++;
			});
	}
